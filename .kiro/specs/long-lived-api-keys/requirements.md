...
5. THE SQLAlchemy mirror (`mintkey-models/mintkey_models/db.py`) SHALL be regenerated from the post-migration schema; the CI mirror-diff gate MUST pass. The corresponding Pydantic models (`ServiceApiKeyCreate`, `ServiceApiKey`, `ServiceApiKeyCreated`) SHALL be added to `mintkey-models`.
