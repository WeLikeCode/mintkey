...
- [ ] 2.1 — Create `apps/` directory (R-2.1)
- [ ] 2.2 — `git mv <each-service>/ apps/<service>/` for all 11 services (R-2.2)
- [ ] 2.3 — `git rm` empty `services/` directory after the 4 services move out (R-2.3)
- [ ] 2.4 — Update `docker-compose.yml` and `docker-compose.test.yml` build contexts to `./apps/<service>` (R-2.4)
- [ ] 2.5 — Audit Dockerfiles for paths above build context (typically none — verify) (R-2.5)
- [ ] 2.6 — Update `Makefile` targets referencing old paths (R-2.6)
- [ ] 2.7 — Update `.github/workflows/*` `paths:` filters and `working-directory:` (R-2.7)
- [ ] 2.8 — Update `go.work` `use` directives for broker/proxy-plugin/kong-syncer/vault-adapter (R-2.8)
- [ ] 2.9 — Verify Go module identity unchanged (no go.mod path renames required) (R-2.9)
- [ ] 2.10 — Update Python workspace path assumptions (R-2.10)
- [ ] 2.11 — Update tests that hardcoded paths (R-2.11)
- [ ] 2.12 — Verification: `docker compose -f docker-compose.yml config` and `-f docker-compose.test.yml config` exit 0 (R-2.12)
