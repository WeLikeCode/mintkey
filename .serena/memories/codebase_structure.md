...
│   │   ├── Dockerfile                  # build context: repo root (needs packages/python/mintkey-models)
│   │   ├── requirements.txt
...
│   │   ├── Dockerfile                  # build context: ./apps/mock-backend
│   │   ├── pyproject.toml
...
│   ├── vault-adapter/                  # Vault Adapter — credential store (Go, gRPC port 8084)
│   ├── kong-syncer/                    # Kong DB-less config sync (Go, port 8085)
│   ├── proxy-plugin/                   # Kong go-pdk egress plugin (Go, port 8086)
...
├── docs/architecture/                  # SOURCE OF TRUTH — 20 ADRs, contracts, flows
│   ├── 01-architecture/adr/            # 0001-0020 accepted ADRs
