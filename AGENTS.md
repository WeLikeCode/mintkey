...
> **Authoritative project instructions for any coding-agent session in this repository.** Read this first; it overrides defaults.
>
> This file follows the [agents.md](https://agents.md) open standard and is intended for opencode, Codex CLI, Cursor agent, Continue, Aider, amp, and other agents that read `AGENTS.md`. Claude Code users get the same content from [`CLAUDE.md`](CLAUDE.md) (the two files are kept in lock-step). Edit both when guardrails change.
...
This repo is the **implementation phase**. Code is generated from the architectural specs (Kiro-driven, spec-driven, test-driven). The Phase 1 task list is in [`.kiro/specs/mintkey-mvp/tasks.md`](.kiro/specs/mintkey-mvp/tasks.md), with milestones `M1.0` → `M1.13`.
...
- **Spawn parallel sub-tasks** for independent multi-perspective reviews when the problem warrants it (security, alignment, syntax, performance — different lenses, different findings). The exact mechanism depends on your agent runtime: opencode's task-mode subagents, Codex's parallel sessions, Claude Code's `Agent` tool, etc.
...
## Karpathy's four principles for coding agents
...
| Phase | Apply |
|---|---|
| **Investigation** | Boil the ocean. Read everything. Run every relevant validator. Spawn parallel sub-tasks for parallel reviews. Don't ship until you've verified. |
...
- **Admin UI**: AdminJS 7.x + Express + `@adminjs/sql` + `passport-openidconnect` + `connect-pg-simple` + `pino` + `vitest` + `pnpm`. **All writes route to FastAPI** ([ADR-0013](docs/architecture/01-architecture/adr/0013-adminjs-pin.md), [ADR-0014.5](docs/architecture/01-architecture/adr/0014-iter-1-2-corrections.md)).
...
.kiro/                          # Kiro spec workspace (specs, steering rules)
  specs/mintkey-mvp/            # Phase 1: requirements.md, design.md, tasks.md (M1.0..M1.13)
  steering/                     # project-wide steering rules
.claude/                        # Claude Code config + skills (skills/task-implement/SKILL.md)
.opencode/                      # opencode config (parallel structure if/when added)
AGENTS.md                       # this file (open standard)
CLAUDE.md                       # Claude Code-flavored sibling (kept in lock-step)
...
- [agents.md](https://agents.md) — the open standard for agent instruction files; supported by opencode, Codex CLI, Cursor agent, Continue, amp, Aider, and others.
...
*Last updated: 2026-05-10. Update this file (and CLAUDE.md, in lock-step) when an architectural decision changes a guardrail.*
