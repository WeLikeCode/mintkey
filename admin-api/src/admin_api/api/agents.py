...
def _wire_id_to_uuid(wire_id: str, prefix: str) -> str:
    ...
    Two wire forms are in use (ADR-0017.11):
      - <prefix><32 hex chars>  — serialised by _agent_row_to_dict / list endpoints
      - <prefix><26 Crockford base32 chars> — generated by _new_agent_id / _new_perm_id / etc.

    Raises ValueError if the wire_id looks like a prefixed ID but cannot be decoded.

    Source: ADR-0017.11; R8.
    """
    if wire_id.startswith(prefix):
        tail = wire_id[len(prefix):]
        if len(tail) == 32:
            # Hex form: agent_<uuid-without-dashes>  (from _agent_row_to_dict)
            try:
                return (
        ...
        if len(tail) == 26:
            # Crockford base32 ULID form: agent_<26 chars>  (from _new_agent_id)
            try:
                val = 0
                for ch in tail.upper():
                    val = (val << 5) | _CROCKFORD.index(ch)
                # val is 130 bits (26 * 5); ULID uses bottom 128 bits
                val &= (1 << 128) - 1
                hex128 = f"{val:032x}"
                return (
