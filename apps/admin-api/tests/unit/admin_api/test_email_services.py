...
class TestCreateEmailServiceCrossTenant403:
    """
    AC-2 verification: create_email_service with a session scoped to TENANT_A
    hitting TENANT_B's path → 403.

    Before fix: no require_tenant_session dep — any operator could POST to any tenant.
    After fix:  require_tenant_session raises HTTPException(403) before handler body runs.

    Uses FastAPI TestClient so Depends() is actually resolved.
    """

    @pytest.mark.asyncio
    async def test_create_email_service_cross_tenant_session_403(self) -> None:
        from fastapi import FastAPI
        from fastapi.testclient import TestClient
        from unittest.mock import AsyncMock, patch as _patch
        from types import SimpleNamespace

        from admin_api.api.email_services import router as svc_router
        from admin_api.db.deps import get_db_session
        from admin_api.services.vault_client import get_vault_client
        ...
        test_app = FastAPI()
        test_app.include_router(svc_router)

        # DB and vault overrides — should never be reached (403 fires first)
        async def _override_session():
            yield _make_session()
        ...
        with _patch("admin_api.auth.sessions.validate_session", new_callable=AsyncMock, return_value=ctx), \
            ...
            with TestClient(test_app, raise_server_exceptions=False) as client:
                resp = client.post(
                    f"/v1/tenants/{TENANT_B_ID}/email-services",
                    json={
                        "provider": "gmail",
