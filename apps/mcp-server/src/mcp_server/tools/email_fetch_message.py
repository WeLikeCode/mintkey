"""
MCP email_fetch_message tool.

GET /v1/tools/email_fetch_message?email_service_id=<id>&mailbox=INBOX&message_id=<uid>

Fetches a single message from an IMAP mailbox.

Implementation:
  1. Auth check.
  2. Permission check (email_permission_grants).
  3. Broker JWT exchange.
  4. GET /v1/email-proxy/messages/{message_id}?service_id=<id>&mailbox=<mailbox>.
...
@router.get("/email_fetch_message")
async def email_fetch_message(
    request: Request,
    ...
    mailbox : str
        The mailbox name (default: INBOX).
    """
