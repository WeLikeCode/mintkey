...
func TestCacheHit(t *testing.T) {
	...
	c.Put("tenant_01", "svc_01", 1, wrappedDEK, encPayload, 1, false, "", "", "", "", "", "", false)
...
func TestCacheExpiry(t *testing.T) {
	...
	c.Put("tenant_01", "svc_01", 1, []byte("dek"), []byte("payload"), 0, false, "", "", "", "", "", "", false)
...
func TestCacheInvalidateByServicePair(t *testing.T) {
	c := cache.New(5 * time.Minute)
	c.Put("tenant_01", "svc_01", 1, []byte("dek_v1"), []byte("pay_v1"), 0, false, "", "", "", "", "", "", false)
	c.Put("tenant_01", "svc_01", 2, []byte("dek_v2"), []byte("pay_v2"), 0, false, "", "", "", "", "", "", false)
	c.Put("tenant_01", "svc_02", 1, []byte("dek_other"), []byte("pay_other"), 0, false, "", "", "", "", "", "", false)
...
func TestCacheHitsMissesGetters(t *testing.T) {
	c := cache.New(5 * time.Minute)
	c.Put("t", "s", 1, []byte("dek"), []byte("enc"), 0, false, "", "", "", "", "", "", false)
	c.Get("t", "s", 1)  // hit
	c.Get("t", "s", 2)  // miss
	c.Get("t", "s", 3)  // miss
