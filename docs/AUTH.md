...
- **admin-ui signing keypair.** The signed-request JWT pattern (admin-ui → admin-api, Ed25519-signed per ADR-0019) requires admin-ui to hold a private key. The admin-ui signing keypair is generated by the seed-job at bootstrap. Under MINTKEY_ENV=production, unsigned write requests are rejected (401).
...
`admin-ui/src/lib/signed-request.ts` implements the signer; admin-api's `AdminUiSignedRequestMiddleware` validates. **Pre-alpha caveat**: in dev the signer gracefully returns null and the header is omitted; admin-api accepts unsigned writes. Under MINTKEY_ENV=production the seed-job generates the keypair and unsigned writes are rejected (401). See "Known gaps" above.
