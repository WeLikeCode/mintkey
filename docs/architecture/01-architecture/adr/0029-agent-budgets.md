...
- New Liquibase changeset `028-budget-counters.yaml` with RLS policy.
...
- [ADR-0006](0006-token-format-and-binding.md) — adds budget check as step 10 in the proxy verification flow.
- [ADR-0010](0010-change-channel-postgres-listen-notify.md) — reuses `mintkey:agent` channel for budget events.
- [ADR-0016](0016-round-2-corrections.md) — extends `Constraints` closed schema with `budget` property.

## Related

- [P-011](../../proposal/P-011-agent-budgets.md) — Original proposal.
- [ADR-0008](0008-multi-tenancy-row-level-with-db-tier.md) — RLS on budget_counters.
