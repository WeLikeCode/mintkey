...
- [`0019-admin-ui-bff-and-write-auth.md`](0019-admin-ui-bff-and-write-auth.md) — Accepted — 2026-05-12. **AdminJS is a BFF over the admin‑api REST API; write authentication via session cookie + signed Ed25519 JWT.** AdminJS holds no DB connection: no `@adminjs/sql`, no `pg`, no `connect‑pg‑simple`. All reads relay the `mintkey_session` cookie to admin‑api; admin‑api owns RLS and the session. State‑changing calls require BOTH the cookie AND a signed Ed25519 `AdminUiSignedRequest` JWT — they must agree (`jwt.sub == session.operator_id` and `jwt.tnt == session.tenant_id`); the effective identity (tenant‑context GUC, audit `actor_id`) comes from the session, never the JWT. `GET /v1/auth/whoami` is part of this contract. Amends [ADR‑0013](0013-adminjs-pin.md) / [ADR‑0014](0014-iter-1-2-corrections.md) §14.5–14.6 / [ADR‑0016](0016-round-2-corrections.md) §16.1. Carries ADR‑0016.1 (`jti` denylist).
