...
- [`P-010-extended-token-class.md`](P-010-extended-token-class.md) → [`ADR‑0018`](../01-architecture/adr/0018-classical-service-api-keys.md) (Proposed) for **Option E** (classical static service API keys), plus an ADR‑0006 follow‑up note for **Option A** (automatic token refresh + `refresh_at` hint). Long‑lived agent access: Option A adopted for agents; Option E adopted for non‑agent clients; the extended‑JWT class (Option C) and the refresh‑token pattern (Option B) rejected. The `long-lived-api-keys` Kiro spec is re‑scoped to Option E.
