...
All of the following containers must be `Up`: `mintkey-admin-api-1`, `mintkey-mcp-server-1`, `mintkey-broker-1`, `mintkey-vault-adapter-1`, `mintkey-proxy-plugin-1`, `mintkey-kong-1`.
...
For local dev use `admin@mintkey.internal` with the password written to the bootstrap volume on first start:
...
2. Click **Create from template** — a modal opens listing bundled service templates
3. Click the **GitHub** card
4. The form pre-fills: `name=GitHub`, `base_url=https://api.github.com`, `auth_scheme=bearer_token`, `description`, and `openapi_url` (GitHub OpenAPI spec URL)
...
**Via UI:** in the ServiceCreateForm fill all fields including the credential value and click **Test connection** — a result panel shows `status_code`, `latency_ms`, and the response body.
...
**Via UI:** on the service show page click **Set Credential**. Choose `bearer_token`, paste the PAT in the Value field, click **Save**.
...
```bash
# 5a. Create the agent — API key is returned ONCE; save it immediately
AGENT=$(curl -s -X POST \
...
Replace `<AGENT_KEY>` with the value from 5a. For remote deployments replace `localhost:8082` with the actual hostname.

> **Tip:** the **Connect your LLM via MCP** button on the admin UI dashboard opens a modal that generates this snippet with the correct URL — just copy and paste.
...
Expected list_services response:
...
```bash
# Describe GitHub — returns description + openapi_url (new fields in OPS-X / OPS-D)
...
> **Proxy URL:** always use `MINTKEY_PROXY_URL` (default: `http://localhost:8000`) — not `MINTKEY_KONG_URL`. The env var is set in `docker-compose.yml` and should be used in any automation or agent system prompt.
...
| 403 `permission_not_found` | Missing grant for agent → service → `call` action | `docker compose logs mintkey-admin-api-1 --tail 50`; check `audit_events` table for `agent.permission.denied` | Create the grant — Step 5b |
...
| 502 from proxy | vault-adapter unreachable or credential row missing | `docker compose logs mintkey-vault-adapter-1 --tail 50` | Verify vault-adapter is up; verify credential was saved in Step 4 |
...
| `description`/`openapi_url` missing from describe_service | Admin UI show page override not in bundle | `docker compose logs mintkey-admin-api-1 --tail 20` | Confirm service was registered with `description` and `openapi_url` fields — re-save if not |
...
# If exposed: open http://localhost:9090
```
