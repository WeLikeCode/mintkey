...
- **A new `http_digest` auth scheme** (enum **17** — next free value): credential payload `{public_key, private_key}`; the proxy attaches a per-request RFC 2617 Digest `RoundTransport` (vetted `github.com/icholy/digest`) to the reverse proxy instead of injecting a static header.
