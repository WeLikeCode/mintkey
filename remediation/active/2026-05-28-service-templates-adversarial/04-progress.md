...
- R18 (2026-05-29): **FULL BRANCH DEPLOYED to local stack** + verified. Rebuilt admin-api/vault-adapter/proxy-plugin (+admin-ui earlier), `up -d --no-deps` (postgres id 823b3bd5 UNCHANGED throughout, no `-v`, data 2/3 preserved). Hit a 3rd deploy-time config gap: FIX-10's DNS-resolving base_url SSRF blocks internal compose hostnames (`mock-backend` → private IP → 422 forbidden_destination). Resolved with the documented dev opt-out `MINTKEY_SSRF_ALLOW_PRIVATE=1` on admin-api in dev compose (`d41c64e`, DEV ONLY; prod leaves SSRF on). **OIDC smoke now 5/5 PASS end-to-end** — validates Step 4 admin-api→scoped-vault PutCredential (BUG-20 live) + Step 10 proxy egress→scoped-vault GetCredential (FIX-1 live). 13 templates live; 2 services/3 agents intact. Deploy commits: 595a2da (BUG-20), e715ae1 (smoke OIDC), d41c64e (dev SSRF flag).

## SESSION COMPLETE. Branch `feature/service-templates` @ 34 commits, local (NOT pushed). Full branch deployed + e2e-verified on local stack; data intact. Non-blocking housekeeping follow-up: /test endpoints' 7 DNS/SSRF sandbox failures, `metrics.go:110` vet, `go.work` non-existent remote.
