...
| AC-2 | **Backend skeleton**: `apps/email-proxy/` builds (`go build ./...`), unit tests `go test ./... -short` green, Dockerfile produces a runnable image. Module path `github.com/mintkey/mintkey/services/email-proxy`. `go.work` updated. | amber — C-2 round 1 REJECTED by reviewer: (1) FATAL vault.pb.go rawDesc binary descriptor not regenerated — `AuthScheme(14).String()` returned "14", `Descriptor().Values().ByNumber(14)` returned nil; (2) MEDIUM JWKS ForceRefresh zeroed lastFetch unconditionally, bypassing 30s rate-limit (DoS amplification). C-2 round 2 dispatched (see round history). |
| AC-3 | **Auth + security primitives**: JWT validation via broker JWKS (Ed25519, force-refresh on unknown kid per ADR-0016.2), RFC 5322 address parser rejects `\r\n` injection, domain allowlist enforced, rate limiting via Postgres advisory locks. Red-team tests pass (`tests/security/test_email_proxy_redteam.py`). | green (pending C-FINAL) — C-3 security primitives landed: addr.go, allowlist.go, sanitize.go, ratelimit.go, smtp_headers.go. 39 unit tests pass; CRLF+oversized red-team cases for every user-input path. In-memory rate limiter with C-9 PG TODO marker. |
| AC-4 | **REST API + 9 endpoints**: all endpoints in design.md §"REST API Design" implemented and reachable on :8088 with brokered-JWT auth. Permission checks enforce `read:email` / `send:email` / `write:email` / `delete:email`. Handler unit tests cover success + 4xx paths. | green (pending C-FINAL) — C-7 landed 9 handlers + 21 unit tests; AuditEmitter DI; no stub-501 routes remain |
| AC-5 | **IMAP/SMTP clients + connection pool**: per-service IMAP pool (default 5 conns, 5-min idle), UIDVALIDITY tracking, SMTP per-op connect. OAuth2 (Gmail + Outlook) with singleflight refresh. Mock-provider integration tests green. | green — C-4 (IMAP pool) + C-5 (SMTP) landed; C-6 OAuth2 singleflight refresh landed + C-6↔C-9 contract alignment applied (Wave-2.5 fixup) |
| AC-6 | **Audit + observability**: emits all 13 event types listed in design.md §10 via `auditq.Queue` → admin-api → Postgres with per-tenant hash chain (`prev_hash`+`hash`). Search queries logged length-only; recipient counts only; subject truncated to 100 chars. Prom metrics + OTel spans wired with attribute allowlist. | green — C-9 emits email.oauth2.refreshed + email.oauth2.expired audit events (NFR-17 verified); C-6↔C-9 contract alignment (Wave-2.5) ensures audit events fire on correct wire path |
| AC-7 | **Admin-API + DB**: `email_services` table created via Liquibase migration with FK to `services`, RLS policy applied. CRUD + OAuth2 authorize/callback endpoints land. Services + email_services insert in single transaction (AC-10.6 / AC-11.10). | green — C-9 landed Liquibase 022-email-services + 023-oauth2-state, RLS strengthened in Wave-2.5 (T-RLS-2a/2b); HTTP-transport-boundary tests added |
| AC-8 | **Admin-UI**: `email_services` AdminJS resource with list/detail/create/edit/delete, OAuth2 setup component with cryptographic `state` CSRF protection, re-authorize flow on `auth_expired`, intro section. Vitest + Playwright happy-path green. Red-team tests (C-13): 22 Go tests proving no credential leakage to logs/audit/OTel/HTTP-error; integration tests: 8 tests with real imapmemserver; architecture tests: 12 Python static checks. | green (C-13 red-team + integration + architecture pending C-FINAL) |
| AC-9 | **Infra + docs + integration**: `email-proxy` container in `docker-compose.yml`, `.ghcr.yml`, `.test.yml`; container-scan matrix entry; `install.sh` + `dev-install.sh` + `.env.example` updated; HOW-TO.md section; ADR README index entry; CHANGELOG; agent-bootstrap.md mentions email scopes; Grafana dashboard imports clean. Full `make dev` → register Gmail (mock) → agent sends email → audit event visible. | green (pending C-FINAL) — C-12 landed: HOW-TO §6, ADR README index entry, ADR-0024 corrigendum (OQ-1/2/3/4 + B1/B2 + S1), container-view D2, grafana/dashboards/email-proxy.json, agent-bootstrap.md `<email_services>` block, CHANGELOG v0.3.0-preview.1, AGENTS.md + CLAUDE.md + KIRO.md updated. C-11 landed: email-proxy service in all 3 compose files, container-scan matrix, install.sh + dev-install.sh secrets, .env.example port vars. |
...
| **C-8** | **Audit emitter + metrics + tracing + wire-up in main.go**. auditq.Queue integration (Event struct, prev_hash/hash chain), all 13 audit event types, Prom metrics, OTel spans (allowlist-enforced). Wire all components in `cmd/email-proxy/main.go`, graceful shutdown. | `apps/email-proxy/internal/{audit,metrics,trace}/*.go`, `apps/email-proxy/cmd/email-proxy/main.go` | C-7 | 1.5d | green (pending C-FINAL) | 1 |
...
- 2026-06-02 — C-2 IMPLEMENTER (round 1, commit 7618e19): Go skeleton landed. apps/email-proxy/ created with cmd/email-proxy + cmd/email-proxy-health + internal/{config,auth,vault,server}. 30 unit tests pass (go test ./... -short). vault.pb.go updated with AUTH_SCHEME_EMAIL_PASSWORD=14/EMAIL_OAUTH2=15/EMAIL_APP_PASSWORD=16. go.work updated. Dockerfile builds 20.7 MB distroless image. ssh-proxy + vault-adapter + admin-api regressions: 0. Smoke: binary starts, warns on unreachable vault, shuts down gracefully on SIGTERM.
- 2026-06-02 — C-2 REVIEWER (round 1): FAIL. Two defects: (1) FATAL — vault.pb.go rawDesc binary descriptor not updated; hand-patch added constants + name/value maps but not the binary blob. AuthScheme(14).String()="14"; Descriptor().Values().ByNumber(14)=nil. All reflection-based serialisation (prototext/protojson/gRPC reflection/OTel) broken for values 14/15/16. The claim "protoc not available" was false — both `/opt/homebrew/bin/protoc` and `~/go/bin/protoc-gen-go` are installed. (2) MEDIUM — JWKS ForceRefresh zeroed lastFetch unconditionally before calling Refresh(), bypassing the 30s rate-limit entirely. Unknown-kid spray = one outbound GET per request (amplification DoS). AC-2 reverted to amber.
- 2026-06-02 — C-2 IMPLEMENTER (round 2): Real protoc regen via `PATH="$HOME/go/bin:$PATH" protoc --go_out=. --go_opt=module=github.com/mintkey/mintkey --go-grpc_out=. --go-grpc_opt=module=github.com/mintkey/mintkey -I docs/architecture/contracts/vault-adapter docs/architecture/contracts/vault-adapter/vault.proto`. Proto go_package updated from `internal/vault/v1` to `packages/go/vault/v1` to match the actual import path. rawDesc binary descriptor now includes EMAIL_PASSWORD(14), EMAIL_OAUTH2(15), EMAIL_APP_PASSWORD(16). ADR-0021→ADR-0022 comment drift auto-corrected. TestAuthSchemeReflection added to descriptor_test.go asserting String() + ByNumber() + ByName() for 14/15/16. JWKS ForceRefresh fixed: added lastForceRefresh field + 5s min-spacing guard; TestForceRefreshThrottle asserts ≤2 JWKS fetches for 100 unknown-kid validations. Makefile proto-gen target added (idempotent). All tests pass.
- 2026-06-02 — C-3 IMPLEMENTER: security primitives landed. NEW package apps/email-proxy/internal/security/ with addr.go (RFC 5322 strict parser, CRLF+control-char+length enforcement), allowlist.go (exact + wildcard domain allowlist, case-insensitive, empty=allow-all), sanitize.go (ScrubBodyForLog → `<scrubbed:N bytes,M lines>`), ratelimit.go (in-memory sliding-window per (agent,service) + C-9 PG advisory-lock TODO), smtp_headers.go (SanitizeHeader — CRLF+length+lowercase). 39 unit tests pass (go test ./internal/security/... -short -count=1 -v), every user-input function has a CRLF + oversized red-team case. No new deps. Commit 8cbbfc3.
- 2026-06-02 — C-5 IMPLEMENTER: SMTP client + MIME builder + send landed. NEW package apps/email-proxy/internal/smtp/. Per-op connect (no pool), TLS enforced (STARTTLS + implicit 465), PLAIN + XOAUTH2 auth, go-message MIME, attachments up to 25MB, multiple recipients, address validation via security.rfc5322. 9 unit tests pass. Commit bfd0382.
- 2026-06-02 — C-6 IMPLEMENTER: OAuth2 access-token cache + refresh delegation landed. NEW package apps/email-proxy/internal/oauth2/. GetAccessToken(tenant, service), singleflight dedup, 90%-TTL cache eviction, ErrRefreshTokenRevoked on 401. 9 unit tests pass. Commit 4509c3d. NOTE: C-6 had a HARD BLOCKER (discovered by Wave-2 batch auditor): refresh sent JSON body {tenant_id, service_id, refresh_token} with no auth header; admin-api expected query params + X-Mintkey-Service-Token header + empty body. See Wave-2.5 entry below.
- 2026-06-02 — C-9 IMPLEMENTER: admin-api email-services + OAuth2 flow landed. Two Liquibase migrations (022-email-services, 023-oauth2-state). email_services.py with 4 endpoints. NFR-17 audit payloads verified via AST grep test (14 total pytests pass). Commit c91ed36. NOTE: C-9's tests called the refresh handler as a Python function with kwargs — HTTP transport boundary (query-vs-body binding) was never exercised. See Wave-2.5 entry below.
- 2026-06-02 — Wave-2.5 fixup (C-6↔C-9 contract alignment): HARD BLOCKER resolved. Pre-fix: email-proxy POST body {tenant_id, service_id, refresh_token} → admin-api returns 401 (no service token) + NFR-17 violation (refresh_token on wire). Fix path B applied: C-6 now sends query params + X-Mintkey-Service-Token header + empty body. MINTKEY_EMAIL_PROXY_SERVICE_TOKEN added to email-proxy config (required, fail-fast). 4 new contract-parity httptest tests added (TestContractParity_*). C-9 side: 3 HTTP TestClient tests (T-HTTP-1/2/3) + 2 RLS strengthening tests (T-RLS-2a/2b). Plan AC-5/6/7 flipped to green. .env.example updated with MINTKEY_VAULT_EMAIL_PROXY_IDENTITY_ID + MINTKEY_VAULT_EMAIL_PROXY_TOKEN (previously missing). Validation: go vet ./... clean; go test ./... -short -count=1 all 9 packages pass (now 16 oauth2 tests + 11 config tests); pytest tests/unit/admin_api/ 165 passed (was 160); ruff check src/ clean.
- 2026-06-02 — C-7 IMPLEMENTER (commit 5c40ada): REST handlers wired. NEW package apps/email-proxy/internal/server/handlers/ with email.go (AuditEmitter DI interface + NoopAuditEmitter, 9 handler methods, credential resolution, rate-limiting, scope checks, CRLF injection block, ScrubBodyForLog in audit payloads) + handlers_email_test.go (21 unit tests). server.go rewritten: stub-501 handleStub replaced by proper mux dispatch to EmailHandlers; withJWTAuth now injects Claims into context via handlers.ClaimsContextKey; newWithHandlers() added for test injection. server_test.go updated: newTestServer uses newWithHandlers + noopPool/OAuth2/Vault/SMTP stubs; TestHandleStub_Returns501 replaced by TestEmailRoutes_RequireAuth (4 sub-tests). Validation: go vet ./... clean; go test ./... -short -count=1 all 10 packages pass (21 new handler tests); docker build mintkey-email-proxy:wave3 succeeded; grep confirms 0 stub-501 responses on email routes; no refresh_token/access_token/client_secret in audit payloads; admin-api 165 pytest still pass; ssh-proxy go build clean.

- 2026-06-02 — C-11 IMPLEMENTER: compose + container-scan + install wiring landed. email-proxy service stanza added to all 3 compose files (ports 8091:8088/8092:8090 — host 8088 reserved for cAdvisor). vault-adapter gets MINTKEY_VAULT_EMAIL_PROXY_IDENTITY_ID/_TOKEN env vars in both docker-compose.yml and docker-compose.ghcr.yml. admin-api gets MINTKEY_EMAIL_PROXY_SERVICE_TOKEN. container-scan.yml matrix += email-proxy row. install.sh generates MINTKEY_VAULT_EMAIL_PROXY_TOKEN + MINTKEY_EMAIL_PROXY_SERVICE_TOKEN via openssl rand -hex 32; post-install banner mentions email-proxy. dev-install.sh env-writer appends the same 2 secrets. .env.example adds MINTKEY_EMAIL_PROXY_HTTP_PORT=8091 + MINTKEY_EMAIL_PROXY_METRICS_PORT=8092 comment block. tools/validate-test-override.py updated: LOCALLY_BUILT_SERVICES += email-proxy. Validation: 4 YAML files clean; bash -n OK; validate-test-override.py ✓; docker compose config --quiet exit 0; docker compose build email-proxy succeeded (CACHED); admin-api 165 pytest pass.
- 2026-06-02 — C-12 IMPLEMENTER (round 1, parent 56e1837): All docs + dashboard + skill + meta files landed. (1) docs/HOW-TO.md §6 added (~270 lines): Adding an email service (3 auth schemes), OAuth2 setup workflow, granting agent permissions, 9 MCP tools table, operational notes. Old §6..§11 renumbered §7..§12. (2) docs/architecture/01-architecture/adr/README.md: ADR-0024 index entry added. (3) docs/architecture/01-architecture/adr/0024-email-proxy-support.md: corrigendum block appended (OQ-1/2/3/4 + B1/B2 + S1 resolutions + services.base_url divergence). (4) docs/architecture/01-architecture/02-container-view.md: email-proxy added as D2 in data-plane subgraph + 5 diagram edges. (5) grafana/dashboards/email-proxy.json: NEW — 6 panels. (6) apps/mcp-server/skills/agent-bootstrap.md: `<email_services>` block added. (7) CHANGELOG.md: v0.3.0-preview.1 entry. (8) AGENTS.md + CLAUDE.md: email service row in How-to-add-an-X table. (9) KIRO.md: email-proxy Quick-links entry. (10) 00-plan.md: AC-9 → green, C-12 status → green, this round history entry. No Go/Python/TS source touched.
- 2026-06-02 — C-8 IMPLEMENTER (round 1, parent 56e1837): Audit emitter + metrics + tracing + main.go wire-up landed. NEW packages: (1) apps/email-proxy/internal/audit/ — Emitter wrapping auditq.Queue, Emit() stamps event_id/at via ulid, increments mintkey_email_proxy_audit_events_emitted_total, scrubs payloads per NFR-17; 14 unit tests (per-event shape + NFR-17 forbidden-key guard). (2) apps/email-proxy/internal/metrics/ — 6 Prometheus counters/histograms/gauges (mintkey_email_proxy_requests_total, request_duration_seconds, oauth2_refresh_total, rate_limit_throttled_total, active_connections, audit_events_emitted_total) via promauto; 6 unit tests. (3) apps/email-proxy/internal/trace/ — OTel span helpers with 6 email.* attrs from allowlist + WithEmailSpan done-fn; 11 unit tests. MODIFIED: cmd/email-proxy/main.go — auditq.New()+Replay()+Start() boot, real audit.NewEmitter wired, OTel shutdown ordered, blank import of metrics package to trigger promauto init; internal/server/server.go — New() accepts handlers.AuditEmitter param (nil→noop fallback), loggingMiddleware now calls emailmetrics.RecordRequest(); internal/config/config.go — AuditWALPath field + MINTKEY_EMAIL_PROXY_AUDIT_WAL_PATH env var. Validations: go vet ./... clean; go test ./... -short -count=1 all 13 packages pass (31 new tests); docker build mintkey-email-proxy:c8 succeeded; smoke /metrics shows mintkey_email_proxy_requests_total{endpoint="list_mailboxes",status="401"} 1 after one unauthenticated request; NoopAuditEmitter gone from main.go; NFR-17 grep on audit/ finds only comments/test-guards; ssh-proxy go build clean; admin-api 165 pytest still pass.
- 2026-06-02 — C-13 IMPLEMENTER (round 1): Final pre-merge defense layer landed. THREE deliverables: (1) Red-team (apps/email-proxy/tests/security/leak_redteam_test.go): 22 Go tests planting LEAK-MARKER-* in 5 leak vectors (body, attachment, refresh_token, access_token, client_secret) × 4 observation points (slog, audit payload, HTTP error JSON, vault zero-on-use) — all 22 PASS. (2) Integration (apps/email-proxy/tests/integration/ with //go:build integration): 8 tests using real imapmemserver + httptest OAuth2 mock + SMTP stub; covers list_mailboxes, list_messages, send_message, audit payload leak check, metrics counter, OAuth2 revoked→401, ScrubBodyForLog, multi-handler audit events — all 8 PASS. README.md documents build-tag invocation. (3) Architecture (apps/admin-api/tests/acceptance/test_email_proxy_architecture.py): 12 Python static checks: AST-grep audit_emit payloads (no refresh_token/access_token/client_secret), email-proxy internal no MINTKEY_OAUTH2_* env reads, email_services RLS enabled (Liquibase YAML parse), oauth2_state RLS enabled, all 4 JWT-guarded route prefixes verified in server.go, health routes NOT JWT-wrapped, span-attrs MD/Go/Python parity (6 canonical email.* attrs), internal refresh endpoint uses service-token header auth — all 12 PASS. Full admin-api suite: 177 passed (165 existing + 12 new). go vet ./... + go vet -tags=integration ./... clean. docker build mintkey-email-proxy:c13 succeeded.
